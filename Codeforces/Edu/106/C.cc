...
int main() {
  ...
  while (t --) {
    ...
    LL bst = n * (c[0] + c[1]);
    array<LL, 2> mx{{c[0], c[1]}}, sm{{0, 0}}, cnt{{0, 0}};

    for (int s = 2; s < n; s ++) { // at most use s segments
      if (c[s] < mx[s&1]) { sm[s&1] += mx[s&1]; mx[s&1] = c[s]; }
      else { sm[s&1] += c[s];  }
      cnt[s&1] ++;

      LL sum = sm[0] + sm[1] + mx[s&1] * (n-cnt[s&1]) + mx[1-(s&1)] * (n-cnt[1-(s&1)]);
      bst = min(sum, bst);
