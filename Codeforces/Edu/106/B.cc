...
int main() {
  ...
  while (t --) {
    ...
    int adj_zero = -1; // the last adjacent 00.
    for (int i = 0; i < (int)s.length() - 1; i ++)
      if (s[i] == s[i+1] && s[i] == '0') adj_zero = i;
    int adj_one = -1; // the first adjacent 11.
    for (int i = 0; i < (int)s.length() - 1; i ++)
      if (s[i] == s[i+1] && s[i] == '1') { adj_one = i; break; }
    
    if (adj_zero != -1 && adj_one != -1 && adj_zero > adj_one)
      cout << "NO\n";
    else
      cout << "YES\n";
  }
