...
int main() {
  ...
  while (t --) {
    array<int, 4> s; for (int &i : s) cin >> i;
    // check if any pair contains the two highest skills.

    array<int, 4> ord{{0, 1, 2, 3}};
    sort(ord.begin(), ord.end(), [&](int i, int j) { return s[i] < s[j]; });

    // ord[2] and ord[3] should from different groups
    if (equal(ord[2], ord[3], 0, 1) || equal(ord[2], ord[3], 2, 3))
      cout << "NO\n";
    else
      cout << "YES\n";
  }
